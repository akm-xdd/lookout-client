...
import { X, Copy, CheckCircle, AlertCircle, Clock, Zap, Globe, Calendar } from 'lucide-react'
      ...
      <div className="relative bg-black border border-white/20 rounded-xl w-full max-w-3xl mx-4 max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="space-y-4">
            <h3 className="text-lg font-semibold border-b border-white/10 pb-2">Request Details</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <div className="px-3 py-2 bg-white/5 border border-white/10 rounded-lg flex items-center space-x-2">
                  <span className={`px-2 py-1 rounded text-xs font-medium border ${getMethodColor(result.method)}`}>
                    {result.method}
                  </span>
                  <span className="text-white break-all text-sm">{result.url}</span>
                </div>
              </div>

              <div>
                <label className="block text-sm text-gray-400 mb-1">Expected vs Actual</label>
                <div className="px-3 py-2 bg-white/5 border border-white/10 rounded-lg">
                  <span className="text-white font-mono text-sm">
                    {result.expected_status} → {result.status_code || 'Error'}
                  </span>
                </div>
              </div>

              <div>
                <label className="block text-sm text-gray-400 mb-1">Test Time</label>
                <div className="px-3 py-2 bg-white/5 border border-white/10 rounded-lg flex items-center space-x-2">
                  <Calendar className="w-4 h-4 text-gray-400" />
                  <span className="text-white text-sm">{formatTimestamp(result.timestamp || Date.now())}</span>
                </div>
              </div>

              <div>
                <label className="block text-sm text-gray-400 mb-1">Timeout Config</label>
                <div className="px-3 py-2 bg-white/5 border border-white/10 rounded-lg flex items-center space-x-2">
                  <Clock className="w-4 h-4 text-gray-400" />
                  <span className="text-white text-sm">{result.timeout_seconds || 30}s timeout</span>
                </div>
              </div>
            </div>
          </div>
          ...
          {result.error && (
            <div className="space-y-4">
              <h3 className="text-lg font-semibold border-b border-white/10 pb-2 text-red-400">Error Details</h3>
          ...
          {/* Response Headers */}
          {(result.response_headers || result.headers) && Object.keys(result.response_headers || result.headers || {}).length > 0 && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-lg font-semibold border-b border-white/10 pb-2">Response Headers</h3>
                <button
          ...
          {result.response_body && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-lg font-semibold border-b border-white/10 pb-2">Response Body</h3>
                <button
                  onClick={() => copyToClipboard(result.response_body || '', 'Response Body')}
                  className="flex items-center space-x-2 px-3 py-1 bg-blue-500/20 hover:bg-blue-500/30 border border-blue-500/30 rounded-lg transition-colors text-sm"
                >
                  <Copy className="w-4 h-4" />
                  <span>{copiedSection === 'Response Body' ? 'Copied!' : 'Copy'}</span>
                </button>
              </div>
              <div className="p-4 bg-white/5 border border-white/10 rounded-lg">
                ...
                {result.response_body.length > 500 && (
                  <p className="text-gray-400 text-xs mt-2">
                    Showing first 500 characters. Use copy button for full response.
                  </p>
                )}
              </div>
            </div>
        ...
        <div className="border-t border-white/10 px-6 py-4 flex items-center justify-between">
          ...
          >
            <Copy className="w-4 h-4" />
            <span>{copiedSection === 'Full Results' ? 'Copied!' : 'Copy Full Results (JSON)'}</span>
          </button>
