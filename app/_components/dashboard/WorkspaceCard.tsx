...
import { MoreVertical, Activity, Clock, AlertCircle } from 'lucide-react'
import { WorkspaceData, formatUptime, formatResponseTime, formatLastCheck } from '@/lib/data-loader'
    ...
    >
      ...
      <div className="grid grid-cols-3 gap-4 mb-4">
        ...
        <div>
          ...
          <div className="text-white font-medium">
            {formatUptime(workspace.uptime)}
          </div>
        </div>
        <div>
          <div className="text-sm text-gray-400 mb-1">Response</div>
          <div className="text-white font-medium">
            {formatResponseTime(workspace.avgResponseTime)}
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between text-xs text-gray-400">
        <div className="flex items-center space-x-1">
          ...
          <span>{formatLastCheck(workspace.lastCheck)}</span>
        </div>
        {workspace.activeIncidents > 0 && (
          <div className="flex items-center space-x-1 text-red-400">
            <AlertCircle className="w-3 h-3" />
            <span>{workspace.activeIncidents} incident{workspace.activeIncidents !== 1 ? 's' : ''}</span>
          </div>
        )}
      </div>
    </div>
